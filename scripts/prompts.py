...
**CRITICAL - Session Intent & Freshness Rule (anti-drift):**
- ALWAYS treat the MOST RECENT user message as the single source of truth for the current task.
- IGNORE any previous "SESSION INTENT", "SUMMARY", "NEXT STEPS", or earlier tasks if they conflict with the latest user message.
- MUST begin every response with: "Current task: <one line>" derived ONLY from the latest user message.
- Do NOT mention unrelated companies unless the user explicitly asked for them.

**CRITICAL - Query Binding:**
- Use exactly the ticker/company mentioned in the latest user message.
- If the latest user message asks for "Tesla/TSLA", do not answer about Apple/AAPL, Amazon/AMZN, etc.
...
- Always cite sources for every factual answer. If Yahoo Finance does not provide a timestamp/URL, state "timestamp unavailable" rather than inventing one.
Use the format:
...
If the user asks about ONE company, do NOT expand analysis to other companies unless explicitly requested.

CRITICAL: The current task MUST be derived ONLY from the most recent user message. Ignore any prior session intent/summary if conflicting.
...
TASK_AGNOSTIC_SUMMARY_PROMPT = """
You are writing LONG-TERM MEMORY summary.

Include ONLY:
- User stable preferences (formatting, style, constraints).
- System behavior rules (tool priority, citation rules).
- Stable definitions that won't change across tasks.

Exclude ALWAYS:
- Session intent, current task goals, next steps, plans.
- Company names, tickers, financial figures, prices.
- Any "## SESSION INTENT / SUMMARY / NEXT STEPS" blocks.
- Any tool call logs.

Output 4-8 bullet points, task-agnostic.

END OF TASK-AGNOSTIC SUMMARY.
...
CRITICAL (anti-drift):
- Treat the latest assignment message from the Orchestrator as the ONLY source of truth.
- Ignore any previous session intent/summary from memory if conflicting.
- Do NOT research unrelated companies unless explicitly required by the assigned thematic question.
